...
import sifive.fpgashells.ip.xilinx.vc707axi_to_pcie_x1.{VC707AXIToPCIeX1, VC707AXIToPCIeX16, VC707AXIToPCIeX1IOClocksReset, VC707AXIToPCIeX1IOSerial}
...
class XilinxVC707PCIeX16(implicit p: Parameters, val crossing: ClockCrossingType = AsynchronousCrossing(8))
  extends LazyModule with CrossesToOnlyOneClockDomain with HasAXI4StreamCrossing
{
  val axi_to_pcie_x16 = LazyModule(new VC707AXIToPCIeX16)
  ...
  val rqmaster: AXI4StreamOutwardNode = axi_to_pcie_x16.rqmaster
  val rcslave: AXI4StreamInwardNode = axi_to_pcie_x16.rcslave
  val ccslave: AXI4StreamInwardNode = axi_to_pcie_x16.ccslave
  val cqmaster: AXI4StreamOutwardNode = axi_to_pcie_x16.cqmaster
  val csrslave: AXI4InwardNode = axi_to_pcie_x16.csrslave

  val intnode: IntOutwardNode = axi_to_pcie_x16.intnode
  ...
  lazy val module = new LazyRawModuleImp(this) {
    ...
    io.port <> axi_to_pcie_x16.module.io.port

    //PCIe Reference Clock
    val ibufds_gte2 = Module(new IBUFDS_GTE2)
    axi_to_pcie_x16.module.io.REFCLK := ibufds_gte2.io.O
    ibufds_gte2.io.CEB := UInt(0)
    ibufds_gte2.io.I := io.port.REFCLK_rxp
    ibufds_gte2.io.IB := io.port.REFCLK_rxn
  }
