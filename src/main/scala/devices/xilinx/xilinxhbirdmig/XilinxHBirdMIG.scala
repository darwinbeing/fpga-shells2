...
import sifive.fpgashells.ip.xilinx.hbirdmig.{HBirdMIGIOClocksReset, HBirdMIGIODDR, hbirdmig}

case class XilinxHBirdMIGParams(
  address : Seq[AddressSet]
)

class XilinxHBirdMIGPads(depth : BigInt) extends HBirdMIGIODDR(depth) {
  def this(c : XilinxHBirdMIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxHBirdMIGIO(depth : BigInt) extends HBirdMIGIODDR(depth) with HBirdMIGIOClocksReset

class XilinxHBirdMIGIsland(c : XilinxHBirdMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth==0x10000000L),"hbirdmig supports 256MiB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxHBirdMIGIO(depth)
    })
    ...
    val blackbox = Module(new hbirdmig(depth))
...
class XilinxHBirdMIG(c : XilinxHBirdMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxHBirdMIGIsland(c, crossing))
